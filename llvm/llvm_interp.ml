...
let rec interp ((p, ctx): prog * typdefs) (globs: value Varmap.t) =
  ...
  let rec interp_int
    ...
    let do_branch l =
      interp_int currfunc prevlabel
         (try Varmap.find l currfunc.f_labels
          with Not_found -> raise (RuntimeError ("Invalid label: " ^ l,
                                                 loc))
         )
         regs globs stack heap
    in
    ...
    | ILabel newlabel ->
       let rec eatphis regs globs pc =
        ...
        | _ -> interp_int currfunc newlabel pc
                 regs
                 globs
                 stack
                 heap
    ...
    | ICast (d, _, _, v, _) ->
       (* Since we're representing everything as an int, casts are no-ops *)
       let (regs, globs) = update d (interp_raw_value v) in
       interp_int currfunc prevlabel (pc + 1) regs globs stack heap
    | IBinop (d, b, _, v1, v2) ->
